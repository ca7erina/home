...
//        lowestDrawdown(array);
    ...
    public static double getDrawdown(double peak, double trough){
        double drawdown = 100*(Math.abs(peak -trough)/Math.abs(peak));// calculate troughdown
        return drawdown;
    }
    ...
    public static void highestDrawdown(double[][] array){ // get the biggest drawdown of every company
        ...
        for(int i =1;i<array[1].length;i++){ // i is company index
            ...
            for(int j =2; j<array.length-1;j++){ // index start from the second date; end at the last second date

                if(array[j][i]>array[j-1][i]&&array[j][i]>array[j+1][i]){ //found peak
                    peak = array[j][i];
                    peakDateIndex = j;
                    gotPeak =true;
                    continue;
                ...
                if(array[j][i]<array[j-1][i]&&array[j][i]<array[j+1][i]&&gotPeak==true){ //found trough
                    trough = array[j][i];
                    troughDateIndex = j;

                    drawdown =getDrawdown(peak, trough);// calculate troughdown
                    if(drawdown>maxDD){
                        maxDD = drawdown;
    ...
    public static void lowestDrawdown(double[][] array){ // get the biggest drawdown of every company
        ...
        for(int i=1;i<5;i++){
            ...
            System.out.println("lowest drawdown:"+ lowestDDList[i][3]);
        }
