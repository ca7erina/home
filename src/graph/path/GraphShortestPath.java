...
//       for(int i=0;i<size;i++){ //loop through all possible starting positions: start from A
        ...
        Vertex1[] vertices = new Vertex1[size]; //initiate the vertex objects
        for(int j=0;j<size;j++){
            vertices[j]=new Vertex1(original[0].split("\t")[j]);
        }
...
class Vertex1{ //everything you need for a vertex
    ...
    public Vertex1 (String labelin){ //give the vertex a name when instantiated
        label=labelin;
    }
}